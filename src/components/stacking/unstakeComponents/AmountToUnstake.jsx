...
const AmountToStake = (props) => {
  ...
  const { hanldeActiveStackTab, activeStackTab ,  unstakeAmount , setUnstakeAmount , stakingBalance} = props;
  ...
  return (
    <>
      <div className="stack-tab">
        <h4 className="text-center text-capitalize letter-3">Please enter the amount of BUSDX you want to unstake</h4>
        <div className="form-group amount-to-unstack-sec">
          <label className="custom-label ml-0 text-capitalize text-center">Amount</label>
          <input type="text" className="form-control" placeholder={auth.BUSDXTokenBalance} name="title" required="" value={unstakeAmount}
            onChange={(e) => setUnstakeAmount(Number(e.target.value))} />
        </div>
        <div className="stackamount-to-unsatck">
          <a href="#" onClick={(event) => { event.preventDefault(); setUnstakeAmount(Number(stakingBalance))}}>
            MAX
          </a>
        </div>
