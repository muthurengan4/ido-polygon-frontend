...
const Checkpoint = (props) => {
  const { hanldeActiveStackTab, activeStackTab , termsCondition , handleTermsConditons , stakingBalance } = props;
  ...
  const [checklistItems, setChecklistitems] = useState([
    ...
    {
      name: "BUSDX Available to unstake",
    ...
    },
    {
      ...
      status:
        auth.authStatus && stakingBalance > 0 && auth.ethBalance > 0,
    },
  ]);
              ...
              <div className="d-flex flex-column">
                ...
                {index == 1 && (
                  <p className="mt-0 letter-no-spacing text-justify">
                    {Number(stakingBalance).toLocaleString(undefined, {maximumFractionDigits:5})} 
                  </p>
                )}
                {index == 2 && (
                  <p className="mt-0 letter-no-spacing text-justify">
